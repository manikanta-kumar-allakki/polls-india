...
def index(request):
	...
	if not request.user.is_authenticated():
		ratings = Ratings.objects.filter(Q(rate_user=request.META['REMOTE_ADDR']))
		print ratings
	else:
		ratings = Ratings.objects.filter(Q(rate_user=request.user.id))
		print ratings
	context = RequestContext(request,{'request': request,'latest_poll_list': latest_poll_list,'full_name':request.user.username,'user_id':request.user.id,'ratings':ratings})
...
def auth_view(request,poll_id,comment_text):
	...
	if user is not None:
		if p.is_active == True:
			auth.login(request, user)
			response = HttpResponseRedirect(reverse('polls:post_comment', args=(poll.id,comment,)))
			if request.method == 'POST':
				if not request.POST.get('remember_me', None):
					request.session.set_expiry(0)
					if username in request.COOKIES:
...
def signup(request):
	if request.method == "GET":
		...
		if form.is_valid():
			...
			post = User.objects.create(username=username, email=email,password=password1)
	...
	if request.method == 'POST':
		...
		try:
			if form.is_valid():
				...
				print x.id
				token_obj = Tokens(token_id = id_generator(size=36), user_id_id = x.id)
				print "test6"
				token_obj.save()
				...
				print "test"
				subject = 'Thank you'
				message = 'welcome to polls website ,Click here to active your account http://10.90.90.124:8000/%s/verification/' % Tokens.objects.get(user_id_id = x.id)
				from_email = settings.EMAIL_HOST_USER
				to_list = [form.cleaned_data['email']]
				send_mail(subject,message,from_email,to_list,fail_silently=True)
				print "test5"
				messages.warning(request, 'Please Check your Mail to Activate your Account')
...
def detail(request,poll_id):
	...
	if  timezone.now() < poll.expire_date:
		...
		messages.warning(request, 'Sorry! Your requesting poll was Expired, Please try another')
		return HttpResponseRedirect(reverse('polls:index')) 
...
def send_forgotpass(request):
	...
	if request.method == "POST":
		...
		try:
			...
			to_list = [obj.email]
			send_mail(subject,message,from_email,to_list,fail_silently=True)
			messages.warning(request,'Please Check your Mail We have sent an Email regarding reset Password link')
			print 'test6'
			return HttpResponseRedirect(reverse('polls:index'))
		except:
			return render(request,'polls/forgotpass.html',{
...
def password_update(request,user_id):
	...
	if request.method == "POST":
		...
		if pass1==pass2:
			...
			c.update({'user_id':a.id})
			c.update({'username':a.username})
			c.update({'error_message':'The Passwords did not match, please enter same'})
...
def post_ratings(request,user_id):
	...
	if user_id == 'None':
		try:
			u = Ratings.objects.filter(rate_user=request.META.get('REMOTE_ADDR'))
			p = Ratings.objects.filter(Q(poll_id = p_id) & Q(rate_user=request.META.get('REMOTE_ADDR')))
			if u:
				for i in u:
					print i.rate_user
					if i.rate_user==request.META.get('REMOTE_ADDR'):
						...
						if p:
							...
							rating_obj = Ratings(poll_id=p_id,rate_user=request.META.get('REMOTE_ADDR'),rating=rate)
							rating_obj.save()
							break
						break
			else:
				...
				for i in u:
					...
					if i.rate_user==user_id:
						...
						if p:
							...
							print 'store with poll_id'
							rating_obj = Ratings(poll_id=p_id,rate_user=user_id,rating=rate)
							rating_obj.save()
							break
						break
			else:
